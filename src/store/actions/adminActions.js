...
export const createNewUser = (data) => {
    return async (dispatch, getState) => {
        try {
            ...
            console.log('check res of create user', res);
            console.log('check reserror of create user', res.errCode);
            if (res && res.errCode === 0) {
            ...
            dispatch(saveUserFailed());
            console.log('save user failed err', e);
        }
    }
}
...
export const deleteAUser = (userID) => {
    return async (dispatch, getState) => {
        try {
            ...
            dispatch(deleteUserFailed());
            console.log('delete user failed err', e);
        }
    }
}
...
export const editAUser = (data) => {
    return async (dispatch, getState) => {
        try {
            ...
            console.log('check reserror', res.errCode);
            if (res && res.errCode === 0) {
                toast.success("Update the user succeed");
                dispatch(editUserSuccess());
                dispatch(fetchAllUsersStart());
            } else {
                toast.error("update1 the user error");
                dispatch(editUserFailed());
            }
        } catch (e) {
            toast.error("update2 the user error");
            dispatch(editUserFailed());
...
export const fetchTopDoctor = (data) => {
    return async (dispatch, getState) => {
        try {
            ...
            if (res && res.errCode === 0) {
                console.log('check action', res.data);
                dispatch({
...
export const getRequiredDoctorInfor = () => {
    return async (dispatch, getState) => {
        try {
            ...
            console.log('FETCH_REQUIRE_DOCTOR_INFOR_FAILED', e);
            dispatch(fetchRequiredDoctorInforFailed());
        }
    }
}
